      ...
      <div className="max-h-full w-full max-w-[1020px] overflow-y-auto rounded-2xl border border-line2 bg-elevated p-8 shadow-[var(--modal-shadow)]">
        <div className="flex flex-col items-center gap-2.5 text-center">
          <svg width="40" height="40" viewBox="0 0 22 22" fill="none" aria-hidden>
            ...
            <path d="M7 9V6.5a4 4 0 0 1 8 0V9" style={{ stroke: "var(--accent-text)" }} strokeWidth="1.8" />
            <circle cx="11" cy="13.5" r="1.4" style={{ fill: "var(--accent-text)" }} />
          </svg>
            ...
            return (
              <div
                ...
                className={`relative flex flex-col gap-4 rounded-xl border p-6 ${
                  highlight ? "border-accent/60 bg-accent-surface" : "border-line2 bg-surface"
                }`}
              >
                {highlight && (
                ...
                <div
                  className={`flex items-center justify-between rounded-lg border px-3.5 py-2.5 text-sm ${
                    highlight ? "border-accent/35 bg-accent/5 text-soft" : "border-line2 text-muted"
                  }`}
                >
                  <span>Preis pro Analyse</span>
                  <span className={`font-display text-[17px] font-bold ${highlight ? "text-accent-text" : "text-ink"}`}>
